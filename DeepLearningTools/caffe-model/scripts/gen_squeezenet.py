...
def generate_fully_train_val(BatchNorm, a, ratio):
    ...
    network_str += generate_conv_layer(3, 32, 1, 1, 'conv1', 'data', 'conv1')
    if (BatchNorm):
      ...
      network_str += generate_pooling_layer(2, 2, 'MAX', 'pool1', 'conv1_bn', 'pool1')
    else:
    ...
    network_str += generate_typeA(idx, 'pool1', str(idx) + '/end', a[0]/ratio, a[0], BatchNorm)
    idx += 1

    network_str += generate_typeB(idx, str(idx - 1) + '/end', str(idx) + '/end', a[0]/ratio, a[0], BatchNorm)
    idx += 1

    network_str += generate_pooling_layer(2, 2, 'MAX', 'pool2', str(idx - 1) + '/end', 'pool2')

    network_str += generate_typeA(idx, 'pool2', str(idx) + '/end', a[0]/ratio, a[0], BatchNorm)
    idx += 1
...
def main():
    ...
    network_str = network_str.replace('{{batch_size_train}}', '64')
    network_str = network_str.replace('{{batch_size_val}}', '1')
    network_str = network_str.replace('{{dropout}}', '4')
