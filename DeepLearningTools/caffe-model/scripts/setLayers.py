...
caffe_path = 'F:/HandPose/CPM_Hands/scripts/python'
import sys, os
sys.path.insert(0, os.path.join(caffe_path))
import caffe
...
def setLayers_twoBranches(data_source, batch_size, layername, kernel, stride, outCH, label_name, transform_param_in, deploy=False, batchnorm=0, lr_mult_distro=[1,1,1]):
    ...
    # produce data definition for deploy net
        ...
        n.data, n.tops['label'] = L.CPMHandData(data_param=dict(backend=1, source=data_source, batch_size=batch_size), 
                                                    cpm_hand_transform_param=transform_param_in, ntop=2)
...
if __name__ == "__main__":
    ...
    if(exp == 1):
        ...
        transform_param = dict(stride=8, crop_size_x=256, crop_size_y=256,np_in_lmdb=21, num_parts=np)
        nCP = 3
        ...
        for nc in range(0,1):
            ...
            outCH =     [64]*3 + [128]* 3  +  [256] * 4 + [256]  +  [512] * 2 + [256] + [128] + [ 0 ] + [128] * 3  + [512] +[np] + [ 0 ] # first-stage
