...
def prepare_models(LAYER, R, NET_PATH, NET_NAME, INPUT_DIM):
    ...
    net = caffe.Net(NET_PREFIX + '_deploy.prototxt', NET_PREFIX + '.caffemodel', caffe.TEST)
    
    fast_net = caffe.Net(NET_PREFIX + '_accelerated_deploy.prototxt', NET_PREFIX + '.caffemodel', caffe.TEST)

    l = ind - 1 #layer index in deploy version
    print l
    print  len(net.blobs[LAYER].data)
    weights = net.blobs[LAYER].data[0]
    bias = net.blobs[LAYER].bias
