...
namespace caffe {
	...
	template<typename Dtype>
	void CPMHandDataTransformer<Dtype>::SetAugTable(int numData){
	...
	template<typename Dtype>
	void CPMHandDataTransformer<Dtype>::TransformMetaJoints(MetaData& meta) {
	...
	template<typename Dtype>
	void CPMHandDataTransformer<Dtype>::TransformJoints(Joints& j) {
		//transform joints in meta from np_in_lmdb (specified in prototxt) to np (specified in prototxt)
		Joints jo = j;
	...
	template<typename Dtype> CPMHandDataTransformer<Dtype>::CPMHandDataTransformer(const CPMHandTransformationParameter& param, Phase phase) : param_(param), phase_(phase) {
		...
		LOG(INFO) << "CPMHandDataTransformer constructor done.";
		np_in_lmdb = param_.np_in_lmdb();
		np = param_.num_parts();
		is_table_set = false;
	}

	template<typename Dtype> void CPMHandDataTransformer<Dtype>::Transform(const Datum& datum, Dtype* transformed_data) {
	...
	template<typename Dtype> void CPMHandDataTransformer<Dtype>::Transform(const Datum& datum, Blob<Dtype>* transformed_blob) {
		const int datum_channels = datum.channels();
		const int datum_height = datum.height();
		const int datum_width = datum.width();
	...
	template<typename Dtype> void CPMHandDataTransformer<Dtype>::Transform_nv(const Datum& datum, Blob<Dtype>* transformed_data, Blob<Dtype>* transformed_label, int cnt) {
	...
	template<typename Dtype> 
	void CPMHandDataTransformer<Dtype>::
		Transform_nv(const Datum& datum, 
	...
	template<typename Dtype>
	void CPMHandDataTransformer<Dtype>::generateLabelMap(Dtype* transformed_label,
		Mat& img_aug, MetaData meta) {
		...
		for (int i = 40; i < 61; i++)
		{
			Point2f center = meta.joint_self.joints[i - 40];
			if (meta.joint_self.isVisible[i - 40] <= 1){
		...
		for (int g_y = 0; g_y < grid_y; g_y++){
			for (int g_x = 0; g_x < grid_x; g_x++){
				...
				transformed_label[(61)*channelOffset + g_y*grid_x + g_x] = max(1.0 - maximum, 0.0);
			}
		}
		int thre = 1;
		for (int i = 0; i < 20; i++){
	...
	template <typename Dtype>
	void caffe::CPMHandDataTransformer<Dtype>::ResizeToFixedSize(Mat& img, Mat& img_aug, MetaData& meta)
	{
		float fixed_width = param_.crop_size_x();
	...
	template<typename Dtype>
	void CPMHandDataTransformer<Dtype>::
		ReadMetaData(MetaData& meta,
		const string& data,
		size_t offset3,
		size_t offset1) { //very specific to genLMDB.py
		...
		for (int i = 0; i < np_in_lmdb; i++){
			DecodeFloats(data, offset3 + 4 * offset1 + 4 * i, &meta.joint_self.joints[i].x, 1);
			DecodeFloats(data, offset3 + 5 * offset1 + 4 * i, &meta.joint_self.joints[i].y, 1);
			//meta.joint_self.joints[i] -= Point2f(1, 1); //from matlab 1-index to c++ 0-index
			float isVisible;
			DecodeFloats(data, offset3 + 6 * offset1 + 4 * i, &isVisible, 1);
			if (isVisible == 0){
	...
	// include mask_miss
	template<typename Dtype>
	float CPMHandDataTransformer<Dtype>::augmentation_scale(Mat& img_src, Mat& img_temp, Mat& mask_miss, Mat& mask_all, MetaData& meta, int mode) {
	...
	template<typename Dtype>
	Size CPMHandDataTransformer<Dtype>::augmentation_croppad(Mat& img_src, Mat& img_dst, Mat& mask_miss, Mat& mask_miss_aug, Mat& mask_all, Mat& mask_all_aug, MetaData& meta, int mode) {
	...
	template<typename Dtype>
	bool CPMHandDataTransformer<Dtype>::augmentation_flip(Mat& img_src, Mat& img_aug, Mat& mask_miss, Mat& mask_all, MetaData& meta, int mode) {
		bool doflip;
	...
	template<typename Dtype>
	float CPMHandDataTransformer<Dtype>::augmentation_rotate(Mat& img_src, Mat& img_dst, Mat& mask_miss, Mat& mask_all, MetaData& meta, int mode) {
	...
	template<typename Dtype>
	float CPMHandDataTransformer<Dtype>::augmentation_scale(Mat& img_src, Mat& img_temp, MetaData& meta) {
		float dice = static_cast <float> (rand()) / static_cast <float> (RAND_MAX); //[0,1]
		float scale_multiplier;
		//float scale = (param_.scale_max() - param_.scale_min()) * dice + param_.scale_min(); //linear shear into [scale_min, scale_max]
		if (dice > param_.scale_prob()) {
	...
	template<typename Dtype>
	bool CPMHandDataTransformer<Dtype>::onPlane(Point p, Size img_size) {
	...
	template<typename Dtype>
	Size CPMHandDataTransformer<Dtype>::augmentation_croppad(Mat& img_src, Mat& img_dst, MetaData& meta) {
		float dice_x = static_cast <float> (rand()) / static_cast <float> (RAND_MAX); //[0,1]
		float dice_y = static_cast <float> (rand()) / static_cast <float> (RAND_MAX); //[0,1]
		int crop_x = param_.crop_size_x();
		int crop_y = param_.crop_size_y();
	...
	template<typename Dtype>
	void CPMHandDataTransformer<Dtype>::swapLeftRight(Joints& j) {
		if (np == 56){
	...
	template<typename Dtype>
	bool CPMHandDataTransformer<Dtype>::augmentation_flip(Mat& img_src, Mat& img_aug, MetaData& meta) {
		bool doflip;
		if (param_.aug_way() == "rand"){
			float dice = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
			doflip = (dice <= param_.flip_prob());
	...
	template<typename Dtype>
	void CPMHandDataTransformer<Dtype>::RotatePoint(Point2f& p, Mat R){
	...
	template<typename Dtype>
	float CPMHandDataTransformer<Dtype>::augmentation_rotate(Mat& img_src, Mat& img_dst, MetaData& meta) {
	...
	template<typename Dtype>
	void CPMHandDataTransformer<Dtype>::putGaussianMaps(Dtype* entry, Point2f center, int stride, int grid_x, int grid_y, float sigma){
	...
	template<typename Dtype>
	void CPMHandDataTransformer<Dtype>::putVecPeaks(Dtype* entryX, Dtype* entryY, Mat& count, Point2f centerA, Point2f centerB, int stride, int grid_x, int grid_y, float sigma, int thre){
		//int thre = 4;
	...
	template<typename Dtype>
	void CPMHandDataTransformer<Dtype>::putVecMaps(Dtype* entryX, 
		Dtype* entryY, 
		Mat& count, 
		Point2f centerA, 
		Point2f centerB, 
		int stride, int grid_x, int grid_y, float sigma, int thre){
		//int thre = 4;
		centerB = centerB*0.125;
		centerA = centerA*0.125;
		Point2f bc = centerB - centerA;
	...
	template<typename Dtype>
	void CPMHandDataTransformer<Dtype>::visualize(Mat& img, MetaData meta, AugmentSelection as) {
	...
	template<typename Dtype>
	void CPMHandDataTransformer<Dtype>::Transform(const vector<Datum> & datum_vector,
	...
	template<typename Dtype>
	void CPMHandDataTransformer<Dtype>::Transform(const vector<cv::Mat> & mat_vector,
		Blob<Dtype>* transformed_blob) {
	...
	template<typename Dtype>
	void CPMHandDataTransformer<Dtype>::Transform(const cv::Mat& cv_img,
		Blob<Dtype>* transformed_blob) {
	...
	template<typename Dtype>
	void CPMHandDataTransformer<Dtype>::Transform(Blob<Dtype>* input_blob,
		Blob<Dtype>* transformed_blob) {
	...
	template<typename Dtype>
	vector<int> CPMHandDataTransformer<Dtype>::InferBlobShape(const Datum& datum) {
		if (datum.encoded()) {
#ifdef USE_OPENCV
	...
	template<typename Dtype>
	vector<int> CPMHandDataTransformer<Dtype>::InferBlobShape(
...
#ifdef USE_OPENCV
	template<typename Dtype>
	vector<int> CPMHandDataTransformer<Dtype>::InferBlobShape(const cv::Mat& cv_img) {
		const int crop_size = param_.crop_size();
		const int img_channels = cv_img.channels();
		const int img_height = cv_img.rows;
		const int img_width = cv_img.cols;
	...
	template <typename Dtype>
	void CPMHandDataTransformer<Dtype>::InitRand() {
	...
	template <typename Dtype>
	int CPMHandDataTransformer<Dtype>::Rand(int n) {
		CHECK(rng_);
		CHECK_GT(n, 0);
		caffe::rng_t* rng =
	...
	template <typename Dtype>
	void CPMHandDataTransformer<Dtype>::clahe(Mat& bgr_image, int tileSize, int clipLimit) {
		Mat lab_image;
	...
	template <typename Dtype>
	void CPMHandDataTransformer<Dtype>::dumpEverything(Dtype* transformed_data, Dtype* transformed_label, MetaData meta){
	...
	INSTANTIATE_CLASS(CPMHandDataTransformer);
